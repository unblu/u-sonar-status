...
@TestProfile(SonarSecretDefinedApprovalModeProfile.class)
class USonarStatusApprovalModeTest {
	...
	void testSonarEndpointRapidReturn() throws Exception {
		...
		wireMockHelper.setupGetMrApprovals();
		wireMockHelper.setupApproveMr();
	...
	@Test
	void testSonarSuccessCaseAndApproved() throws Exception {
		wireMockHelper.setupGetMr();
		wireMockHelper.setupGetMrApprovalsAlreadyApproved();
		...
		given().when()
				...
				.body("sonar_event_uuid", notNullValue())
				.body("gitlab_project_id", equalTo(56))
				.body("gitlab_merge_request_iid", equalTo(100))
				.body("error", nullValue());
	...
	@Test
	void testSonarFailedQualityGate() throws Exception {
		wireMockHelper.setupGetMr();
		wireMockHelper.setupGetMrApprovalsAlreadyApproved();
		wireMockHelper.setupUnapproveMr();

		SonarEventSimple simpleSonarEventFailedQualityGate = MockUtil.createErrorSonarEventSimple();

		given().when()
				.header("Content-Type", "application/json")
				.body(simpleSonarEventFailedQualityGate)
				.post("/u-sonar-status/sonar-replay")
				.then()
				.statusCode(Response.Status.OK.getStatusCode())
				.body(startsWith("{\n"))
	...
	@Test
	void testSonarFailedQualityGateAndUnapproved() throws Exception {
		wireMockHelper.setupGetMr();
		wireMockHelper.setupGetMrApprovals();
